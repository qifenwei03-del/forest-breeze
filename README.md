...
## Motion field 編輯器（按 A）

按 **A** 進入／離開編輯模式。圖片上會疊一層 12 × 12 的向量場：

- **箭頭方向** = 該區域的風向
- **箭頭長度與顏色** = 振幅倍率（灰 = 不動、綠 = 1×、橘 = 2×）
- **在圖上拖曳** = 筆刷。滑動的方向就是塗上去的風向，所見即所得
- 筆刷大小、筆刷振幅、Save / Reset、Field ON 在左下角面板
- 面板下方即時顯示游標所在格的方向與振幅

不在編輯模式時 canvas 是 `pointer-events: none`，完全不影響瀏覽。

### 它改的是什麼

field 存的是**絕對方向**，兩層的軸向偏移（`TIER_AXIS_DEG`：GRAY 0°、BLACK 90°）
仍然疊在這個方向上 —— 所以「GRAY 沿風向、BLACK 垂直於風向」的關係不會被破壞，
你畫的箭頭是「這一區的風往哪吹」。

**Field ON 時 `Wind Direction` 滑桿不再影響畫面**，它只是 Reset 的種子值。

### 為什麼不會製造變形

- 12 × 12 在 2496px 的圖上是每格約 208px，落在「一簇葉子」的尺度
- field texture 是**線性內插**的，格與格之間平滑過渡，方向不會突變
- 振幅倍率 0 的區域直接併進 `gate`，整段位移運算會被跳過

**鋪平的 field（所有格同方向、振幅 1）與關閉 field 的輸出逐像素相同** ——
實測 mean 0 / max 0，所以啟用 field 本身不會改變畫面。

### 儲存

每一筆拖曳結束會自動存進 localStorage，重整不會白畫。
**Save** 另外把 JSON 複製到剪貼簿（也印在 console），格式：

```json
{ "grid": 12, "cells": [x, y, amp, x, y, amp, ...] }
```

**Reset** 把整個場鋪平成目前的 Wind Direction 並清除存檔。

localStorage 只影響你自己那台。要讓所有訪客看到同一個 field，
得把 JSON 交給我寫進原始碼（跟 `DEFAULT_CONFIG` 一樣的流程）。
